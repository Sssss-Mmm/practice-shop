...
public class EventService {
    ...
    private EventResponse toResponse(Event event) {
        return EventResponse.builder()
                ...
                .venue(event.getVenue() != null ? com.example.practice_shop.dtos.ticketing.VenueResponse.builder()
                        .venueId(event.getVenue().getId())
                        .name(event.getVenue().getName())
                        .addressLine1(event.getVenue().getAddressLine1())
                        .addressLine2(event.getVenue().getAddressLine2())
                        .city(event.getVenue().getCity())
                        .state(event.getVenue().getState())
                        .postalCode(event.getVenue().getPostalCode())
                        .seatingChartUrl(event.getVenue().getSeatingChartUrl())
